...
const HeroSection = () => (
  <section className="relative flex min-h-screen flex-col justify-center overflow-hidden bg-white pt-[4.5rem] mesh-green">
    ...
    <div className="relative z-[1] mx-auto grid max-w-7xl items-center gap-12 px-4 py-14 sm:px-6 lg:grid-cols-2 lg:gap-14 lg:px-8 lg:py-20">
      ...
      >
        ...
        >
          Join thousands of aspiring agents who are passing their IRDA exams, building their practice, and changing lives —
          with the tools that actually prepare you.
        </motion.p>
        ...
        >
          <Link
            to="/register"
            className="inline-flex min-h-[48px] flex-1 items-center justify-center rounded-full bg-lic-teal px-7 text-base font-semibold text-white shadow-md transition-all duration-200 ease-material hover:scale-[1.03] hover:shadow-teal-glow sm:flex-none animate-pulse-soft"
          >
            Create free account
          </Link>
          <a
            href="#how-it-works"
            className="inline-flex min-h-[48px] flex-1 items-center justify-center rounded-full border-2 border-lic-teal bg-white/80 px-7 text-base font-semibold text-lic-teal backdrop-blur-sm transition-all duration-200 ease-material hover:bg-lic-mint sm:flex-none"
          >
            See how it works
          </a>
        </motion.div>
        ...
        >
          {TRUST_CHIPS.map((c) => (
            <li
              key={c.label}
              className="inline-flex items-center gap-2 rounded-full border border-black/[0.06] bg-white/90 px-3 py-2 text-xs font-medium text-lic-charcoal shadow-soft sm:text-sm"
            >
...
function LaptopMock() {
  return (
    <div className="rounded-card-lg border border-black/[0.08] bg-white p-3 shadow-card">
      <div className="flex items-center gap-2 border-b border-black/[0.06] px-2 pb-3">
        <span className="h-2.5 w-2.5 rounded-full bg-red-400" />
        <span className="h-2.5 w-2.5 rounded-full bg-amber-400" />
        <span className="h-2.5 w-2.5 rounded-full bg-lic-teal" />
        <span className="ml-2 font-mono text-[10px] text-lic-body/70">app.licpro.academy</span>
      </div>
      <div className="mt-3 grid gap-3 rounded-card bg-lic-offwhite p-4">
        ...
        <div className="h-28 rounded-card bg-white px-3 py-2 shadow-soft">
          <p className="text-[10px] font-semibold uppercase tracking-wide text-lic-body">Performance</p>
          <div className="mt-2 flex h-16 items-end gap-1">
            {[40, 55, 48, 70, 62, 88, 92].map((h, i) => (
              <div
                key={i}
                className="flex-1 rounded-t bg-gradient-to-t from-lic-teal to-lic-sky"
                style={{ height: `${h}%` }}
              />
            ))}
          </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <div className="rounded-card border border-black/[0.06] bg-white p-3 shadow-soft">
